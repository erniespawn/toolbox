...
class Runner(object):
	def __init__(self):
		# ver = os.system("cat /etc/hosts | grep prod | awk '{print $3}'")
		self.passer = []
		self.build()
	...
	def build(self):
		if args.s == '0':
						...
						try:
							...
							if len(st) == 19:
								if st[11] != 'success_esme':
									if st[18] == '':
										self.passer.append(Views(st[11], st[8], st[12], st[0], st[7], 'smsc-2-eu4', st[14], st[15], 'null'))
									else:
										if st[3] == '0':
											pass
										else:
											self.passer.append(Views(st[11], st[8], st[12], st[0], st[7], 'smsc-2-eu4', st[14], st[15], st[18].replace(" ", ""))) #This only adds the failed and temp_failed
			...
			with gzip.open("/home/kennyC/sms/smsc2-eu4/cdr-2021-02-01-08.gz",'rb') as f:
					line_file1 = f.readlines()
					for line in line_file1:
						list = []
						list = " ".join(line.split())
						...
						try:
							...
							if len(st) == 19:
								if st[11] != 'success_esme':
									if st[18] == '':
										pass
										self.passer.append(Views(st[11], st[8], st[12], st[0], st[7], 'smsc-2-eu4', st[14], st[15], 'null'))  #This only adds the success and partial
									else:
										if st[3] == '0':
											pass
										else:
