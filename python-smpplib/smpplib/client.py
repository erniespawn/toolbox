...
class Client(object):
    ...
    def read_pdu(self):
        ...
        try:
            ...
            # print ("Dit is de bbbb length {}".format(length))
        ...
        raw_pdu = raw_len
        # print ("Dit is de eeeeee length {}".format(raw_pdu))

        while len(raw_pdu) < length:
            raw_pdu += self._socket.recv(length - len(raw_pdu))

        # print ("Dit is de ddddd length {}".format(self.logger.debug))
    ...
    def _message_received(self, pdu):
        ...
        try:  # Temp hack to solve status = 15 issue. Need to investigate where this 15 comes from.
            if status == 15:
                status = 0
        except:
            pass
    ...
    def listen(self, ignore_error_codes=None, auto_send_enquire_link=True):
        ...
        t_end = time.time() + 17  # Prog will stay open for 17 seconds then exit
